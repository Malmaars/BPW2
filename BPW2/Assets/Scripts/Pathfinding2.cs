    ...
    public Transform findPath(Vector2 startLoc, Vector2 target)
    {
        ...
        while(currentNode.pos != target)
        {
            ...
            foreach(node neighbour in neighbours)
            {
                ...
                if (closedPoss.Contains(neighbour.pos) || Physics2D.OverlapPoint(neighbour.pos, TileLayer).gameObject.tag != "Walkable" || Physics2D.OverlapPoint(neighbour.pos, TileLayer).gameObject == null)
                {
                    continue;
                }
